...
def main():
    ...
    # The penalization function suggests that a vertex can be in 
    # 10 clusters at most to get a positive penalized score. 
    for v in vertices:
    ...
    score_denominator = 0 # sum of number of vertices in each cluster
    # divide score by this to get potentially more informative metric
    for cluster in filtered_clusters:
        score_denominator += len(cluster)

    # is the score, as reported, particularly informative? Different denominator?
    print "Enrichment score penalizing overlap: {}".format(score/score_denominator)
    print "Raw score: {}".format(score)
    print "Denominator: {}".format(score_denominator)
